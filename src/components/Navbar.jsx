  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[100] flex justify-center p-6 pointer-events-none">
      <nav className="w-full max-w-6xl backdrop-blur-xl bg-black/30 border border-white/5 px-6 md:px-10 py-4 rounded-[30px] flex justify-between items-center shadow-2xl relative pointer-events-auto">
        {/* Logo */}
        <div className="text-xl font-black tracking-tighter italic text-white uppercase">
        ...
        <div className="flex items-center gap-4">
          ...
          <div ref={magneticRef} className="hidden sm:block">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="bg-[#6366F1] text-white px-7 py-3 rounded-full font-bold text-[10px] uppercase tracking-widest transition-colors duration-300 shadow-lg shadow-[#6366F1]/20"
            >
              Resume
            </motion.button>
          </div>
            ...
            >
              {navLinks.map(link => (
                <a
                  ...
                  className="text-gray-400 font-bold uppercase text-[11px] tracking-[0.2em] hover:text-[#6366F1] py-3 border-b border-white/5 transition-colors"
                >
                  {link.name}
                </a>
              ))}
